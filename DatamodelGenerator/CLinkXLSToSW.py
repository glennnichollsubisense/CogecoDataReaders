...
SSHEETCOLUMN_PNIATTRIBUTEPRIORITY = 17
SSHEETCOLUMN_FEATUREPOINTDESCRIPTION = 0
SSHEETCOLUMN_PNIJOINTYPE = 16
SSHEETCOLUMN_PNIJOINTO = 15
SSHEETSTARTTABNO = 3
SSHEETENDTABNO = 130
...
class CLinkXLSToSW():
    ...
    def buildSWField (self, pSheet, pRow):
        ...
        lFieldDefaultValue = pSheet.cell_value(pRow, SSHEETCOLUMN_PNIATTRIBUTEDEFAULTVALUE).strip()
    ...
    def addMappingTagsToEngine(self, pEngine, pFieldManager):
        ...
        for iparameters in pEngine.findall('parameters'):
            ...
            print ("adding mapping for " + lSourceTable + '=>' + lGISTable)
            lMappedFields = pFieldManager.findMappingsFromSourceTable(lSourceTable)  # tablename
            print ('**** ' + repr(len(lMappedFields)) + ' mapped fields found')
            lMappings=ELEMENTTREE.Element('parameter')
            lMappings.set('name', 'field_mappings')
            lMappings.set('comment', 'AutoGenerated from CLinkXLSToSW-' + repr(datetime.date.today().isoformat()))
            for i in lMappedFields:
                print ('!!!!!trying to generate a map element for ' + repr(i.className()) + ":" + repr(lGISTable))
                if i.className()==lGISTable:
    ...
    def updateMigrationXML(self, pfieldManager):
        pfieldManager.showClassesManaged()

        # read the xml file
        lTree = ET.parse (self.s_base_folder + 'runner_config.xml')
        lPNIEngine='not set'
        lEngines1= lTree.findall('./engines')
        for iengine in lEngines1:
            if iengine.attrib['title'].lower().find('input to pni')!=-1:
                lPNIEngine=iengine
                break

        for itypeengine in lPNIEngine.findall('engines'):
            print (itypeengine.attrib['title'])
            for iengine in itypeengine.findall('engine'):
                print (iengine.attrib['name'])
                self.addMappingTagsToEngine(iengine, pfieldManager)
    ...
    def processMainDBWorkBook (self, pFileName):
        ...
        self.updateMigrationXML(self.fieldManager())
...
if __name__== "__main__":
    ...
    lXLSToSW.processMainDBWorkBook ('FiberBase Mapping v3.xls')
